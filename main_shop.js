...
function triggerShiftReport() {
    ...
    showFormModal("🔒 የዕለት ሂሳብ ሪፖርት መዝጊያ ማቅረቢያ", [
        ...
        let alertBox = document.getElementById('shiftStatusAlert');
...
function renderHistoryTable() {
    ...
    let historyList = d.history || []; let filterInput = document.getElementById('historyDateFilter');
        ...
        if(localDB.motors) {
            Object.keys(localDB.motors).forEach(mUser => {
                ...
                if(motor.region === currentTenant.region && motor.zone === currentTenant.zone && motor.woreda === currentTenant.woreda) {
                    if(!motor.activeOrders) motor.activeOrders = [];
                    motor.activeOrders.push({
                        poolId: poolId, shopUsername: currentTenant.username, shopName: currentTenant.shopName,
                        shopPhone: currentTenant.phone, shopMap: currentTenant.googleMapsLink || "-",
                        buyerName: ord.buyerUser, buyerPhone: ord.buyerPhone,
                        buyerMap: ord.mapLink || "-", address: ord.address || "-",
                        itemName: ord.itemName, qty: ord.qty,
                        totalPrice: ord.total, deliveryFee: ord.deliveryFee || 0,
...
function renderApp() {
    ...
    if(d.sessionActive && sessionDisplay) { sessionDisplay.innerText = `📅 ${session.date} | 👤 አስገቢ፡ ${session.employee} | 💰 መነሻ ካዝና፡ ${session.initialFloat} ETB`; }
    ...
    let formattedDateToday = typeof getTodayFormatted === 'function' ? getTodayFormatted() : ""; 
    ...
    let totalInCash = document.getElementById('totalInCash');
    if(totalInCash) totalInCash.innerText = finalCashInHand.toFixed(1) + " ETB";
    ...
    if (currentUserRole === "owner") {
        ...
        let totalCapEl = document.getElementById('totalCapital');
        if(totalCapEl) totalCapEl.innerText = currentTotalCapital.toFixed(1) + " ETB";
        
        let todayNetEl = document.getElementById('todayNetProfit');
        if(todayNetEl) todayNetEl.innerText = (todayProfit - todayExpensesTotal).toFixed(1) + " ETB";
        
        let monthlyNetEl = document.getElementById('monthlyNetProfit');
        if(monthlyNetEl) monthlyNetEl.innerText = monthlyProfit.toFixed(1) + " ETB";
        
        let monthlyExpEl = document.getElementById('monthlyExpenses');
        if(monthlyExpEl) monthlyExpEl.innerText = tExp.toFixed(1) + " ETB";
        
        let totalDrawsEl = document.getElementById('totalDraws');
        if(totalDrawsEl) totalDrawsEl.innerText = tDraw.toFixed(1) + " ETB";
        
        if (typeof myChart !== 'undefined' && myChart) { myChart.data.datasets[0].data = [currentTotalCapital, tSales, todayProfit - todayExpensesTotal]; myChart.update(); }
        renderHistoryTable();
    }
    ...
    if(typeof renderMainCart === 'function') renderMainCart();
    checkTimeLock();
...
function checkMorningSession() {
    ...
    if (!d.sessionActive) {
        showFormModal("የቀኑ መጀመሪያ መመዝገቢያ (የካዝና ማስሞያ)", [
            ...
            let recFilter = document.getElementById('receiptDateFilter');
            if(recFilter) recFilter.value = getTodayFormatted();
            saveAndRefresh();
        });
    } else { renderApp(); }
}
...
function openSellChoiceModal() {
    let searchInp = document.getElementById('inventorySearchInput');
...
window.renderMainCart = function() {
    ...
    if(!container) return;
    if(!mainCart || mainCart.length === 0) { container.innerHTML = ""; if(emptyMsg) emptyMsg.style.display = "block"; if(totalEl) totalEl.innerText = "0"; return; }
...
window.toggleVatReceiptPanel = function() {
    ...
    if(panel) {
        ...
        let dEl = document.getElementById('vatCurrentDateSpan');
        if(dEl) dEl.innerText = typeof getTodayFormatted === 'function' ? getTodayFormatted() : "";
...
window.generateStandaloneVatReceipt = function() {
    ...
    let dateStr = typeof getTodayFormatted === 'function' ? getTodayFormatted() : "";
    ...
    const setElText = (id, text) => { let el = document.getElementById(id); if(el) el.innerText = text; };
    setElText('recPrintShopName', currentTenant.shopName);
    setElText('recPrintFullName', currentTenant.fullName);
    setElText('recPrintBizType', currentTenant.businessType || "አጠቃላይ ንግድ");
    setElText('recPrintTin', currentTenant.tinNumber || "-");
    setElText('recPrintTradeReg', currentTenant.tradeRegistration || "-");
    setElText('recPrintPhone', currentTenant.phone || "-");
    setElText('recPrintEmail', currentTenant.gmail || "-");
    setElText('recPrintRegion', currentTenant.region || "-");
    setElText('recPrintZone', currentTenant.zone || "-");
    setElText('recPrintWoreda', currentTenant.woreda || "-");
    setElText('recPrintKebele', currentTenant.kebele || "-");
    setElText('recPrintHouseNo', currentTenant.houseNo || "-");
    setElText('recPrintCustomerName', cName);
    setElText('recPrintCustomerTin', "-");
    setElText('recPrintDate', dateStr);
    setElText('recPrintReceiptId', "#" + recId);
    ...
    setElText('recPrintSubTotal', subTotal.toFixed(2));
    setElText('recPrintVatPercent', vatRate);
    setElText('recPrintVatAmount', vatAmount.toFixed(2));
    setElText('recPrintGrandTotal', grandTotal.toFixed(2));
    ...
    let rModal = document.getElementById('specialVatReceiptModal');
...
function generateAdvancedReceipt(itemsArray, subTotal, currentSeller, recId = null, saveToHistory = true, givenShopName = null, givenBType = null, buyerName = null, buyerPhone = null, passedVat = null, givenOwnerName = null, givenOwnerPhone = null) {
    ...
    `;
    let tEl = document.getElementById('formModalTitle'); if(tEl) tEl.innerText = "🧾 የሽያጭ ደረሰኝ";
    let bEl = document.getElementById('formModalBody'); if(bEl) bEl.innerHTML = receiptHTML;
    let fEl = document.getElementById('formModalFooter'); if(fEl) fEl.innerHTML = '';
    
    if(typeof openModalContainer === 'function') openModalContainer(); 
    let mEl = document.getElementById('formModal'); if(mEl) mEl.classList.remove('hidden');
}
...
// ማስተካከያ (Safe DOM Element Setting): ሎጊን አድርጎ ገፁን ሲከፍት ምንም እንዳያቋርጥ የተደረገ
function launchApp(tenant) {
    currentTenant = tenant;
    if(typeof switchView === 'function') switchView('appPage');
    
    const setSafeText = (id, text) => { let el = document.getElementById(id); if (el) el.innerText = text; };
    const setSafeValue = (id, val) => { let el = document.getElementById(id); if (el) el.value = val; };
    const toggleVisibility = (id, isVisible) => { let el = document.getElementById(id); if (el) { isVisible ? el.classList.remove('hidden') : el.classList.add('hidden'); } };

    setSafeText('shopTitle', tenant.shopName + (currentUserRole === "staff" ? " (የሰራተኛ ገጽ)" : " (የባለቤት ገጽ)"));
    setSafeText('roleSubTitle', currentUserRole === "staff" ? "🛠️ የተገደበ የሰራተኛ መሸጫ እና መመዝገቢያ ሞድ" : "👑 ሙሉ የሱቅና የኪራይ መቆጣጠሪያ ፓነል");
    setSafeText('profShopName', tenant.shopName);
    setSafeText('profGmail', tenant.gmail || "-");
    setSafeText('profExpiry', tenant.expiryDate ? `${tenant.expiryDate} (${tenant.contractType})` : "ያልተገደበ");
    setSafeText('tenantRentDisplay', (tenant.registrationFee || 0) + " ETB");

    let vatRate = (localDB.adminSettings && localDB.adminSettings.vatRate) ? parseFloat(localDB.adminSettings.vatRate) : 0;
    let rentAmount = parseFloat(tenant.registrationFee) || 0;
    let calculatedVat = (rentAmount * vatRate) / 100;
    setSafeText('tenantVatDisplay', calculatedVat.toFixed(2) + " ETB (" + vatRate + "%)");

    setSafeValue('receiptDateFilter', typeof getTodayFormatted === 'function' ? getTodayFormatted() : "");
    
    let activeTheme = tenant.theme || 'theme-deepblue';
    document.body.className = activeTheme; 
    setSafeValue('themeSelector', activeTheme);
    setSafeValue('inventorySearchInput', "");
    ...
    if (currentUserRole === "staff") {
        toggleVisibility('ownerDashboard', false);
        toggleVisibility('chartContainer', false);
        toggleVisibility('btn_add_item', false);
        toggleVisibility('btn_expense', false);
        toggleVisibility('btn_next_day', false);
        toggleVisibility('btn_clear_all', false);
        toggleVisibility('owner_add_box', false);
        toggleVisibility('btn_settlement', false);
        toggleVisibility('historySection', false);
        toggleVisibility('tenantProfileSection', false);
        if(singleStaffBtn) singleStaffBtn.classList.add('hidden');
    } else {
        toggleVisibility('ownerDashboard', true);
        toggleVisibility('chartContainer', true);
        toggleVisibility('btn_add_item', true);
        toggleVisibility('btn_expense', true);
        toggleVisibility('btn_next_day', true);
        toggleVisibility('btn_clear_all', true);
        toggleVisibility('owner_add_box', true);
        toggleVisibility('btn_settlement', true);
        toggleVisibility('historySection', true);
        toggleVisibility('tenantProfileSection', true);
