...
function loadLocalStorageBackup() {
  ...
  if (backup) {
    ...
    if (parsedBackup.revenueAuthorities)
      localDB.revenueAuthorities = parsedBackup.revenueAuthorities;
    if (parsedBackup.motors) localDB.motors = parsedBackup.motors;
    if (parsedBackup.motorQuotas)
      localDB.motorQuotas = parsedBackup.motorQuotas;
    if (parsedBackup.taxReceipts)
      localDB.taxReceipts = parsedBackup.taxReceipts;
    if (parsedBackup.tariffs) localDB.tariffs = parsedBackup.tariffs;
    if (parsedBackup.businessTypes)
      localDB.businessTypes = parsedBackup.businessTypes;
    ...
    fbRequest
      ...
      .catch((err) => {
        ...
        if (isOnline) {
          ...
          console.warn(
            `Sync failed. Retrying in ${delayTime / 1000} seconds...`
          );
...
const cleanData = (data) =>
  data !== undefined ? JSON.parse(JSON.stringify(data)) : null;
...
function pushRevenueFirebase() {
  if (typeof currentRevenueOfficer !== "undefined" && currentRevenueOfficer) {
    let currentTime = Date.now();
    let revData = cleanData(
      localDB.revenueAuthorities[currentRevenueOfficer.username]
    );
    if (revData) {
      ...
      queueAction(
        "UPDATE",
        "revenueAuthorities",
        currentRevenueOfficer.username,
        revData
      );
    }
    if (localDB.motorQuotas) {
...
function sendAdminTelegramAlert(message) {
  ...
  let tgToken =
    localDB.adminSettings && localDB.adminSettings.tgToken
      ? localDB.adminSettings.tgToken
      : null;
  let tgChatId =
    localDB.adminSettings && localDB.adminSettings.tgChatId
      ? localDB.adminSettings.tgChatId
      : null;
  fetch(backendAPIUrl, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ text: message, token: tgToken, chatId: tgChatId }),
  }).catch((err) => console.log(err));
}

function sendTelegramAlert(message) {
  if (typeof currentTenant === "undefined" || !currentTenant) return;
  fetch("/api/sendTenantTelegram", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ username: currentTenant.username, text: message }),
  }).catch((err) => console.log(err));
}

function sendMotorTelegramAlert(username, message) {
  fetch("/api/sendMotorTelegram", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
if (typeof db !== "undefined") {
  ...
  window.setupSecureUserListeners = function () {
    ...
    function shouldUpdateLocal(incomingData, localData, collection, docId) {
      ...
      let pendingQueue =
        actionQueue ||
        JSON.parse(localStorage.getItem("tirfe_action_queue")) ||
        [];
      if (collection) {
        let hasPendingForThis = pendingQueue.some(
          (a) => a.collection === collection && (!docId || a.docId === docId)
        );
      ...
      let incomingTime =
        incomingData && typeof incomingData.lastUpdated === "number"
          ? incomingData.lastUpdated
          : 0;
      let localTime =
        localData && typeof localData.lastUpdated === "number"
    ...
    if (
      typeof currentTenant !== "undefined" &&
      currentTenant &&
      !window.tenantListenerAttached
    ) {
      window.tenantListenerAttached = true;
      db.ref(`tirfe_system/tenants/${currentTenant.username}`).on(
        "value",
        (snapshot) => {
          if (snapshot.exists()) {
            let incomingData = snapshot.val();
            if (
              shouldUpdateLocal(
                incomingData,
                localDB.tenants[currentTenant.username],
                "tenants",
                currentTenant.username
              )
            ) {
    ...
    if (
      typeof currentBuyer !== "undefined" &&
      currentBuyer &&
      !window.buyerListenerAttached
    ) {
      window.buyerListenerAttached = true;
      db.ref(`tirfe_system/buyers/${currentBuyer.username}`).on(
        "value",
        (snapshot) => {
          if (snapshot.exists()) {
            let incomingData = snapshot.val();
            if (
              shouldUpdateLocal(
                incomingData,
                localDB.buyers[currentBuyer.username],
                "buyers",
        ...
        if (snapshot.exists()) {
          ...
          for (let tUser in incomingTenants) {
            ...
            if (
              shouldUpdateLocal(
                inData,
                localDB.tenants[tUser],
                "tenants",
                tUser
              )
            ) {
              localDB.tenants[tUser] = Object.assign(
                {},
                localDB.tenants[tUser] || {},
                inData
              );
    ...
    if (
      typeof currentRevenueOfficer !== "undefined" &&
      currentRevenueOfficer &&
      ...
      db.ref(
        `tirfe_system/revenueAuthorities/${currentRevenueOfficer.username}`
      ).on("value", (snapshot) => {
        if (snapshot.exists()) {
          let incomingData = snapshot.val();
          if (
      ...
      db.ref(`tirfe_system/tenants`).on("value", (snapshot) => {
        if (snapshot.exists()) {
          ...
          for (let tUser in incomingTenants) {
            ...
            if (
              shouldUpdateLocal(tData, localDB.tenants[tUser], "tenants", tUser)
            ) {
    ...
    if (
      typeof currentMotor !== "undefined" &&
      currentMotor &&
      !window.motorListenerAttached
    ) {
      window.motorListenerAttached = true;
      db.ref(`tirfe_system/motors/${currentMotor.username}`).on(
        "value",
        (snapshot) => {
          if (snapshot.exists()) {
            let incomingData = snapshot.val();
            if (
              shouldUpdateLocal(
                incomingData,
                localDB.motors[currentMotor.username],
                "motors",
                currentMotor.username
              )
            ) {
  ...
  function triggerUIRefresh() {
    ...
    if (typeof currentTenant !== "undefined" && currentTenant) {
      ...
      if (typeof renderTenantTaxReceipts === "function")
        renderTenantTaxReceipts();
    }
