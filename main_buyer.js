...
window.addToBuyerCart = function(shopKey, itemIdx, itemName, price, availableRem) {
    ...
    if(window.buyerCartData && window.buyerCartData.length > 0) {
        if(window.buyerCartData[0].shopKey !== shopKey) {
            let sName = localDB.tenants[window.buyerCartData[0].shopKey] ?
            localDB.tenants[window.buyerCartData[0].shopKey].shopName : "ሌላ ሱቅ";
...
1px dashed #eab308; padding-top: 5px; margin-top: 5px;">ጠቅላላ ሂሳብ (Total): <span id="buyerCartTotalSum" style="color: var(--success-color); font-weight: bold;">${finalTotal.toFixed(2)}</span> ብር</div>
            `;
        } else {
...
id: "mapLink", label: "የጎግል ማፕ ሊንክ (አማራጭ)", type: "text", placeholder: "https://maps.google.com/..." },
            ...
            let transportName = res.transport === 'car' ? '🚗 መኪና' : '🏍️ ሞተረኛ';
            let confirmMsg = `የታዘዙ ዕቃዎች: ${combinedItems}\nየትራንስፖርት: ${transportName}\n-------------------------\nጠቅላላ የዕቃ ሂሳብ: ${grandTotal.toFixed(2)} ETB\n-------------------------\nይህንን ትዕዛዝ ወደ ሻጩ መላክ እርግጠኛ ነዎት?`;
...
= ord.status;
                            ...
                            let cl = st === "pending" ? "text-warning" : (st === "accepted" ? "text-success" : (st === "completed" ? "text-success" 
: "text-danger"));
                            ...
                            if(ord.transport === "motor" && (st === "pending" || st === "accepted")) {
                                ...
                                let isDisabled = ord.deliveryFeePaid > 0 ?
"disabled" : "";
                                ...
                                <div style="margin-top: 8px; display: flex; gap: 5px; align-items: center; background: rgba(0,0,0,0.2); padding: 5px; border-radius: 6px; border: 1px solid rgba(255,255,255,0.05);">
                                    <input type="number" id="delFee_${tKey}_${ord.orderId}" placeholder="የዴሊቨሪ ክፍያ (ብር)" style="width: 130px; padding: 6px; margin: 0; 
font-size: 0.85rem;" value="${feeValue}" ${isDisabled}>
            ...
            carouselHTML += `
                <div class="carousel-item-card" style="flex: 0 0 170px; background: rgba(30, 41, 59, 0.8); border: 1px solid rgba(255,255,255,0.08); border-radius: 8px; padding: 10px; display: flex; flex-direction: column; justify-content: space-between;">
                    <div onclick="viewImageFullscreen('${itemImg}')" style="cursor: pointer;">
                        ...
                        <div style="color: var(--warning-color); font-size: 0.85rem; font-weight: bold;
margin-top: 2px;">${item.price} ETB</div>
                        <div style="color: #94a3b8;
font-size: 0.7rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin-top: 2px;">🏬 ${item.tenant.shopName}</div>
                    </div>
                    <button class="btn-add btn-sm" style="width: 100%;
margin-top: 8px; padding: 8px 0; font-size: 0.85rem; border-radius: 4px; font-weight:bold;" onclick="addToBuyerCart('${item.shopKey}', ${item.originalIdx}, '${item.name}', ${item.price}, ${item.qty - item.sold})">🛒 ወደ ካርት ጨምር</button>
                </div>
            `;
        });
        ...
        <div style="grid-column: 1 / -1; margin-bottom: 12px; margin-top: 5px;"><h3 style="color: #fff; font-size: 1.1rem; margin: 0;
font-weight: 600;">🛍️ አጠቃላይ የዕቃዎች ዝርዝር (All Mixed Products)</h3></div>
        `;
        ...
        allItems.forEach(item => {
            ...
            <div class="shop-card" style="display: flex; flex-direction: column; justify-content: space-between; margin-bottom: 0;">
                <div>
                    <div class="shop-card-header" style="padding-bottom: 6px;
border-bottom: 1px solid rgba(255,255,255,0.05); margin-bottom: 8px; display: flex; align-items: center;
gap: 8px;">
                        <img src="${shopLogo}" class="shop-avatar" onerror="this.src='https://cdn-icons-png.flaticon.com/512/869/869636.png'" style="width:28px;
height:28px; margin:0;">
                        <div class="shop-meta" style="margin:0;">
                            <h3 style="font-size: 0.85rem;
margin:0; line-height:1.2;">${t.shopName}</h3>
                            <span style="color:#64748b;
font-size:0.7rem;">📍 ${t.address || 'ያልተገለጸ'}</span>
                        </div>
                    </div>
                    <div class="catalog-item-card" style="background:transparent;
...
font-size:0.9rem; color:#fff;">${item.name}</span>${modelDisplay}
                            <div style="color:var(--warning-color);
font-weight:bold; margin-top:2px;">${item.price} ETB <small>(${unitLabel})</small></div>
                            <div style="color:#94a3b8;
font-size:0.75rem; margin-top:2px;">ቀሪ፡ ${rem}</div>
                        </div>
                    </div>
                </div>
                <div style="margin-top:12px;">
                    <div 
style="display:flex; gap:5px; margin-bottom:6px;">
                        <button class="btn-success btn-block" style="background:var(--warning-color);
color:#000; font-weight:bold; font-size: 1rem; padding: 10px;" onclick="addToBuyerCart('${item.shopKey}', ${item.originalIdx}, '${item.name}', ${item.price}, ${rem})">🛒 ወደ ካርት ጨምር (Add to Cart)</button>
                    </div>
                    <div class="shop-links" style="display:grid;
grid-template-columns: 1fr 1fr; gap:4px; padding:0; margin:0;">
                        <a href="tel:${t.phone}" class="btn-link-action" style="background:#22c55e;
color:#fff; padding:4px; font-size:0.75rem; text-align:center; border-radius:4px; display:block; text-decoration:none;">📞 ደውል</a>
                        ${tgLink ? `<a href="https://t.me/${tgLink}" target="_blank" class="btn-link-action" style="background:#0088cc;
color:#fff; padding:4px; font-size:0.75rem; text-align:center; border-radius:4px; display:block; text-decoration:none;">✈️ ቴሌግራም</a>` : `<span class="btn-link-action" style="background:#334155; color:#64748b; padding:4px; font-size:0.75rem; text-align:center; border-radius:4px;
display:block;">✈️ የለም</span>`}
                    </div>
                </div>
            </div>`;
...
color:#94a3b8; grid-column: 1/-1; padding:20px;">በተፈለገው ስም የተገኘ ምንም ሱቅ ወይም ዕቃ የለም።</p>';
    }
