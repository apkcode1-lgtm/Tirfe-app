...
window.openBuyerProfileSettings = function() {
    ...
    showFormModal("⚙️ የፕሮፋይል ሲቲንግ", [
        ...
        if(oldU !== newU) {
            ...
            if (typeof db !== 'undefined') { db.ref(`tirfe_system/buyers/${oldU}`).remove(); } // አሮጌው ዳታቤዝ እንዲጠፋ የተጨመረ
        } else { 
            localDB.buyers[newU] = currentBuyer;
        }
...
window.openDeliveryOrderModal = function(shopKey, itemIdx, itemName, price) {
    ...
    showFormModal("🚚 " + itemName + " - ዴሊቨሪ ማዘዣ", [
        ...
        { id: "qty", label: "የሚፈልጉት ብዛት (ግዴታ)", type: "number", 
        defaultValue: "1" },
    ...
    (res) => {
        ...
        // አዲሱ ማስተካከያ፡ የሻጩን ኖድ ቀጥታ ወደ ፋየርቤዝ መላክ
        if (typeof db !== 'undefined') { db.ref(`tirfe_system/tenants/${shopKey}`).set(t).catch(e => console.error(e)); }
...
window.submitDeliveryFee = function(shopKey, orderId) {
    ...
    if(t && t.data && t.data.deliveryOrders) {
        ...
        if(ord) {
            ...
            // አዲሱ ማስተካከያ፡ የሻጩን ኖድ ቀጥታ አፕዴት ማድረግ
            if (typeof db !== 'undefined') { db.ref(`tirfe_system/tenants/${shopKey}`).set(t).catch(e => console.error(e)); }
...
window.checkoutBuyerCart = function() {
    ...
    showCustomConfirm("ትዕዛዝ ማረጋገጫ", "ሁሉንም የቅርጫት ትዕዛዞች ወደየሱቆቹ መላክ ይፈልጋሉ?", () => {
        ...
        for(let sKey in shops) {
            ...
            // አዲሱ ማስተካከያ፡ የሻጩን ዳታቤዝ ቀጥታ ወደ ሻጩ ክላውድ መላክ (የ"ሱቅ ነኝ ግዛ" ትዕዛዝ እንዲታይ)
            if (typeof db !== 'undefined') { db.ref(`tirfe_system/tenants/${sKey}`).set(t).catch(e => console.error(e)); }
        }
...
async function renderBuyerCatalog() {
    ...
    // አዲሱ ማስተካከያ፡ ገዢው ሁልጊዜ አዳዲስ ሪል-ታይም አፕዴቶችን እንዲያይ Length Check ተነስቷል
                    ...
                    t.data.deliveryOrders.forEach(ord => {
                        if(ord.buyerUser === liveBuyer.username) {
                            ...
                            if(ord.transport === "motor") {
                                ...
                                let btnText = ord.deliveryFeePaid > 0 ?
                                "ገብቷል" : "አስገባ";
                                ...
                                <div style="margin-top: 8px; display: flex; gap: 5px; align-items: center; background: rgba(0,0,0,0.2); padding: 5px; border-radius: 6px; border: 1px solid rgba(255,255,255,0.05);">
                                    <input type="number" id="delFee_${tKey}_${ord.orderId}" placeholder="የዴሊቨሪ ክፍያ (ብር)" style="width: 130px; padding: 
                                    6px; margin: 0; font-size: 0.85rem;" value="${feeValue}" ${isDisabled}>
    ...
    if (allItems.length > 0) {
        ...
        carouselItems.forEach(item => {
            ...
            carouselHTML += `
                <div class="carousel-item-card" style="flex: 0 0 170px; background: rgba(30, 41, 59, 0.8); border: 1px solid rgba(255,255,255,0.08); border-radius: 8px; padding: 10px; display: flex; flex-direction: column; justify-content: space-between;">
                    ...
                    margin-top: 8px; padding: 5px 0; font-size: 0.75rem; border-radius: 4px;" onclick="openDeliveryOrderModal('${item.shopKey}', ${item.originalIdx}, '${item.name}', ${item.price})">🚚 እዘዝ</button>
                </div>
            `;
        });
        ...
        margin-bottom: 12px; margin-top: 5px;"><h3 style="color: #fff; font-size: 1.1rem; margin: 0;
            ...
            <div class="shop-card" style="display: flex; flex-direction: column; justify-content: space-between; margin-bottom: 0;">
                ...
                <div style="margin-top:12px;">
                    ...
                    style="display:flex; gap:5px; margin-bottom:6px;">
                        ...
                        background:var(--warning-color); color:#000;" onclick="buyFromShop('${item.shopKey}', ${item.originalIdx}, '${item.name}', ${item.price}, ${rem})">🛒 ሱቅ ግዛ</button>
                    </div>
