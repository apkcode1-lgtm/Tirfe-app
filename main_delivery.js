...
// 8. ኦርደር ሲቀበል (Lock & Link) - የድግግሞሽ ስህተት ሙሉ በሙሉ የተቀረፈበት
window.acceptMotorOrder = function(index) {
    ...
    // ድርብ ክሊክ (Double click protection) ማረጋገጫ 
    ...
    // ----------------------------------------------------------------------
    // የተስተካከለው ዋና የድግግሞሽ መከላከያ (Ghost Order Fix)
    // ሞተረኛው የሻጩን አዲስ ዳታ በቀጥታ ከ Firebase ስቦ ያስተካክላል እንጂ ከ LocalDB አይወስድም
    // ----------------------------------------------------------------------
    if (shopKey) {
        if (typeof db !== 'undefined' && isOnline) {
            let shopOrdersRef = db.ref(`tirfe_system/tenants/${shopKey}/data/deliveryOrders`);
            shopOrdersRef.once('value').then(snap => {
                if (snap.exists()) {
                    let freshShopOrders = snap.val() || [];
                    if (!Array.isArray(freshShopOrders)) {
                        freshShopOrders = Object.values(freshShopOrders);
                    }
                    
                    let orderUpdated = false;
                    freshShopOrders = freshShopOrders.map(o => {
                        // ትክክለኛውን ትዕዛዝ ፈልጎ ሞተረኛውን ይመድባል
                        if (o.orderId == acceptedOrder.orderId || (o.buyerPhone == acceptedOrder.buyerPhone && o.itemName == acceptedOrder.itemName)) {
                            o.motorUser = currentMotor.username;
                            o.status = 'accepted';
                            orderUpdated = true;
                        }
                        return o;
                    ...
                    // ማስተካከያው ከተደረገ ብቻ ወደ ዳታቤዝ አዲስ ዳታውን ይልካል (ድግግሞሽ የለም)
                    if (orderUpdated) {
                        shopOrdersRef.set(freshShopOrders).catch(err => console.error(err));
                    }
                }
